									...
									<td class="cell-action">
										...
										<div class="options">
											<div class="ui small basic icon buttons">
												...
												<a id="removerElemento{{$ingresoExt->id}}"  class="ui button basic red"><i class="trash icon"></i></a>
											</div>
										</div>
									</td>
								</tr>
								@endforeach
								<form action="{{route('guardarIngresoExtraordinario',compact('anio'))}}" method="POST" class="form-inline">
									{{ csrf_field() }}
									<input type="hidden" name="condominio_id" value="{{$condominio->id}}">
									<input type="hidden" name="form" value="table">
									<tr class="agregar active">
										<td>
											<div class="ui input mini {{$errors->ingresoExtraStore->has('concepto') && old('form') == 'table'?' error':''}}">
												<input 	type="text" 
														autocomplete="off" 
														class="input-table" 
														name="concepto" 
										...
										<td>
											<div class="ui input mini {{$errors->ingresoExtraStore->has('cantidad') && old('form') == 'table'?' error':''}}">
												<input 	type="number" 
														autocomplete="off" 
														class="input-table" 
														name="cantidad" 
...
@section('scripts')
	@parent
	@include('removeOption',['elementos'=>$ingresosExtraordinarios,'ruta'=>'eliminarIngresoExtraordinario'])
@endsection